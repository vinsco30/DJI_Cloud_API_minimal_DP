...
## Setup
0. Open the container `dji-cnt`
1. Install dependencies: `pip install -r ./requirements.txt` --- ###DEPRECATED###
2. Install docker and setup `emqx` (MQTT server) --- IT HAS TO RUN IN BACKGROUND
...
4. Set env variable `HOST_ADDR`, `USERNAME`, `PASSWORD` to your IP which will be visible for controller and run `./cloud_api.http.py` ###DEPRECATED IF YOU USE THE DOCKER CONTAINER
5. Set env variable `HOST_ADDR` and run `./cloud_api_mqtt.py`  ###DEPRECATED IF YOU USE THE DOCKER CONTAINER
